...
export default function GenerateSalesReportWeekly({ user }) {
  const [transactions, setTransactions] = useState([
    { id: 1, created_at: "2025-01-01", user: "Admin", action: "Sale", amount: 1500 },
    { id: 2, created_at: "2025-01-07", user: "Admin", action: "Sale", amount: 2800 },
    { id: 3, created_at: "2025-01-10", user: "Admin", action: "Sale", amount: 3200 },
  ]);
  ...
  const saveEditTransaction = (id) => {
    ...
    setTransactions(
      transactions.map((t) => (t.id === id ? { ...t, ...editEntry, amount: Number(editEntry.amount) } : t))
    );
    setEditingId(null);
    setEditEntry({});
  };
  ...
  // Compute week range from a date (Sunday - Saturday)
  ...
  return (
    <AuthenticatedLayout user={user}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 7rem", marginTop: "-1.5rem", marginBottom: "1rem" }}>
        ...
        <Link to="/generate-sales-report" style={{ backgroundColor: "#4b2e17", color: "white", padding: "0.5rem 1.5em", borderRadius: "0.375rem", fontSize: "1rem", fontWeight: "bold", textDecoration: "none" }}
      ...
      <div style={{ maxWidth: "68rem", margin: "2.5rem auto", fontFamily: "sans-serif" }}>
        ...
        <div style={{ marginTop: "-3.4rem", backgroundColor: "#f3e6d9", padding: "1.5rem", borderRadius: "0.75rem", border: "1px solid #d7bfa0", marginBottom: "2rem" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center" }}>
            <button style={{ padding: "0.5rem 1.5rem", backgroundColor: "#4b2e17", color: "#fff", border: "none", borderRadius: "0.375rem", fontWeight: "bold", cursor: "pointer" }}
        ...
        <div style={{ backgroundColor: "#fff", border: "1px solid #d7bfa0", borderRadius: "0.75rem", padding: "1rem", overflowX: "auto", marginBottom: "2rem" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "center" }}>
            <thead style={{ backgroundColor: "#f3e6d9", color: "#4b2e17" }}>
              <tr>
                <th style={{ width: "30%" }}>Start Date - End Date</th>
            ...
            <tbody>
              ...
              {transactions.length ? transactions.map((t) => (
                <tr key={t.id}>
                  <td>
                    {editingId === t.id ? (
                      <input type="date" value={editEntry.created_at} onChange={(e) => setEditEntry({ ...editEntry, created_at: e.target.value })} style={{ padding: "0.3rem", width: "100%", boxSizing: "border-box" }} />
                    ) : (
